...
for kk = 1:length(anatomy_labels)
    ...
    for ii = 1:length(samples)
        ...
        D_inf = [param(idx).D_inf]; D_inf = D_inf(cond);
        cD    = [param(idx).cD]; cD = cD(cond);

        nPoints = length(D_inf);
     
        smpl_X1 = [smpl_X1 D_inf];
        smpl_X2 = [smpl_X2 cD];
    ...
    % exportgraphics(gcf, [d_save, 'cD-Dinf-', anat '-' region, '.png'], 'Resolution', 300)
    title([anat '-' region])
end
...
for kk = 1:length(anatomy_labels)
    ...
    % exportgraphics(gcf, [d_save, 'Dinf-hist-', anat '-' region, '.png'], 'Resolution', 300)
...
for kk = 1:length(anatomy_labels)
    ...
    % title(['$d_{\mathrm{eff}} = \, $', num2str(effect, '%.2f')] ,'interpreter', 'latex','fontsize',10)
    % exportgraphics(gcf, [d_save, 'cD-hist-', anat '-' region, '.png'], 'Resolution', 300)
...
for kk = 1:length(anatomy_labels)
    ...
    iqr_val = iqr(smpl_tbi(:,1));
    binWidth = 0.5*iqr_val / numel(smpl_tbi(:,1))^(1/3);        
    ...
    % title(['$d_{\mathrm{eff}} = \, $', num2str(effect, '%.2f')] ,'interpreter', 'latex','fontsize',10)
    % exportgraphics(gcf, [d_save, 'zD-hist-', anat '-' region, '.png'], 'Resolution', 300)
    title(['$d_{\mathrm{eff}} = \, $', num2str(effect, '%.2f') ' \\ ' [anat '-' region]] ,'interpreter', 'latex','fontsize',10)    
end
